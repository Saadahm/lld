...
//        foodDeliveryService.cancelOrder(order2.getId());
        foodDeliveryService.updateOrderStatus(order2.getId(), OrderStatus.CONFIRMED);



        // Assign Delivery Agent
        foodDeliveryService.assignDeliveryAgent(order2.getId());
        foodDeliveryService.updateOrderStatus(order2.getId(), OrderStatus.DELIVERED);



        Order order3 = foodDeliveryService.placeOrder(customer2.getId(), restaurant1.getId(), List.of("Burger", "Sushi"));
//        foodDeliveryService.cancelOrder(order2.getId());
        foodDeliveryService.updateOrderStatus(order3.getId(), OrderStatus.CONFIRMED);

        // Assign Delivery Agent
        foodDeliveryService.assignDeliveryAgent(order3.getId());
    }
}